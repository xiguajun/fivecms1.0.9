...
class Article extends Common
{
    public function show()
    {
        ...
        /*文章详情start*/
        $r = db('article')->alias('a')->join('__ARTICLE_DATA__ d ', 'a.id= d.id','left')->where('a.id', $id)->where('a.status', 1)->find();
        if (!$r) {
            $this->error('文章不存在');
        }
        $gallery = string2array($r['gallery']);
        $this->assign('gallery', $gallery);
        $this->assign('article', fiveHtmlentitydecode($r));
        /*文章详情end*/
        /*栏目详情start*/
        $catid = $r['catid'];
        ...
        /*seo end*/
        /*模板start*/
        return $this->fetch($this->template . $cateInfo['show']);
        /*模板end*/
    }
}