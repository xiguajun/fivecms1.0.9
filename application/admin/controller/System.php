    ...
    public function set()
    {
        if (request()->isPost()) {
            ...
            if (file_exists($path1)) {
                $arr1 = (include $path1);
                $arr1['template']['view_path'] = './template/pc/' . $data['template_pc'] . '/';
            ...
            if (file_exists($path2)) {
                $arr2 = (include $path2);
                $arr2['template']['view_path'] = './template/wap/' . $data['template_wap'] . '/';
            ...
            db('system')->update($data);
            $this->success('设置成功');
        }
        $detail = db('system')->find(1);
